...
# ¿Está China como partner en la base?
distinct(comtrade_mar, p) |> filter(p == "CHN")

# Si está, ¿qué posición ocupa en el ranking completo (no solo el top 15)?
mar_exp |>
...
#-----------------------
#===============================================================================#
# BLOQUE 7: PERFIL DE COMERCIO EXTERIOR — TABLAS + EVOLUCIÓN
#===============================================================================#

## --- 7.1: Principales bienes exportados e importados ---
...
## --- 7.2: Montos totales de expos/impos (WLD) + evolución + saldo ---
...
## --- 7.3: Principales países a los que les compra/vende cada bien ---
...
## --- 7.4: Ranking de países (exportaciones e importaciones totales) ---

ranking_paises_exp <- mar_exp |>
  filter(p != "WLD", year == 2024) |>
  group_by(p) |>
  summarise(total = first(total_expo)) |>
  arrange(desc(total)) |>
