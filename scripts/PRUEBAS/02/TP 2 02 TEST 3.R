...
#
# RECONCILIACIÓN DE LA FÓRMULA DE VPMgL (ver cabecera de Script 1): ya no
# hay dos fórmulas conviviendo. La función compartida calibrar_mfe_va()
# (definida en Script 1, sub-bloque 2.1) es ahora la única usada en todo
# el trabajo - acá se la reutiliza para construir el Escenario B (OCDE,
# perímetros corregidos) con los mismos share_trabajo/va_sobre_output que
# ya se calculan más abajo (Secciones B y C), y el Escenario A se reusa
# directamente del objeto res_A que ya viene calculado desde Script 1 (no
# se recalcula acá).
#
...
# fórmula que convivía sin resolverse - ver cabecera). share_trabajo y
# va_sobre_output de este bloque (par_fosfatos/par_auto) son exactamente
# los insumos que la Sección E le pasa a calibrar_mfe_va() (Script 1) para
# construir el Escenario B como curva completa, con la misma fórmula que
# el Escenario A: VPMgL(L) = participación laboral observada * VA(L)/L.
...
#===============================================================================#
# L_fosfatos_eq (Script 1, Escenario A: ~127.728, con la fórmula VA
# definitiva) es una extrapolación ~7,5x más allá del empleo observado en
# fosfatos (17.000). Se la compara contra una tecnología de coeficientes
# fijos, calibrada en el mismo punto observado.
...
#===============================================================================#
# HALLAZGO CENTRAL DE ESTA SECCIÓN: el salario real de OCP resultó POR
# ENCIMA de la VPMgL predicha (w/VPMgL ~ 1,31 con la fórmula VA definitiva
# - ver validacion_salario_real más abajo), no muy por debajo como asumían
# los "escenarios hipotéticos" de versiones anteriores del trabajo. Esto
# retracta la narrativa de monopsonio (OCP no subpaga explotando su
# posición de empleador dominante) y la reemplaza por una de RESTRICCIÓN
# DE CAPACIDAD: el enigma no es "por qué paga tan poco" (paga mucho, 11x
# el salario industrial CNSS, y por encima de su propio producto marginal
# estimado), sino "por qué no hay más gente empleada ahí" - y esa pregunta
# ya la respondió el techo técnico de Leontief (Sección F): no hay margen
# técnico para emplear muchos más, sin importar el salario. Que el salario
# supere a la VPMgL estimada (y no solo la iguale) es, si acaso, evidencia
# más fuerte en contra del monopsonio - aunque también puede reflejar que
# participación_laboral_A (promedio HCP del sector minero completo) no
# capture exactamente la de OCP en particular.
...
# VPMgL de fosfatos sobre VA, punto observado (anio_ref_fpp), Escenario A -
# viene de res_A (calibrar_mfe_va(), Script 1), no de un cálculo aparte.
# res_A$vpmgl_f_obs está en millones de MAD por trabajador/año; *1e6/12
# convierte a MAD por trabajador/mes, igual que el resto de las cifras
# salariales de esta sección.
...
# w/VPMgL ~ 1,31: OCP paga POR ENCIMA de su producto marginal estimado, no
# por debajo - descarta con más margen aún la hipótesis de monopsonio. El
# salario real (56.461 MAD/mes en 2023) es 11,3 veces el salario industrial
# de CNSS y ~18 veces el SMIG.

## --- Reinterpretación: ¿qué explica entonces que no haya más empleo? ---
# Si el salario ya iguala o supera la VPMgL, un modelo de monopsonio
...
# Sección F: el techo técnico (~19.627 trabajadores) está apenas por
# encima del empleo actual (17.000), muy por debajo de cualquier expansión
# significativa - sin importar cuánto pague OCP, no hay mucho más margen
# para contratar.
...
resumen_reencuadre_2d <- tibble(
  ...
  evidencia_precio = paste0("w/VPMgL = ", round(w_ocp_2023_mad_mes / vpmgl_f_mad_mes, 2),
                            " (salario por encima de la VPMgL estimada, no por debajo)"),
...
# 4. comparacion_escenarios$brecha_vpmgl_observada -> la brecha final,
#    sobre VA, en los dos escenarios (A-HCP y B-OCDE, perímetros
#    corregidos), para citar en la presentación. Fórmula única en todo el
#    trabajo: participación laboral observada x VA/L (calibrar_mfe_va(),
#    Script 1).
...
#    w/VPMgL ~ 1,31 (salario real POR ENCIMA de la VPMgL estimada, con la
#    fórmula VA definitiva). Retracta el encuadre de monopsonio de
#    versiones anteriores (inversion_parametros, eps/phi implícitos - ya no
#    forman parte del script activo). El inciso 2d se reencuadra como
#    restricción TÉCNICA de capacidad (Leontief, Sección F), no de PRECIO
#    del trabajo.
