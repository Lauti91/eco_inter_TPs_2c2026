...
# PRIMER INTENTO (con bug, dejado documentado): se había multiplicado
# TANTO valor_bruto COMO empleo por la misma fracción - eso cancela la
# fracción algebraicamente y no cambia nada (vpmgl daba idéntico: 4.192 en
# ambos casos). Además, reconstruir el VA de C29+C27 multiplicando
# exportaciones por un ratio era un rodeo innecesario: la OCDE YA PUBLICA
# el VA real de C29+C27 (B1G_total), no hace falta imputarlo. Ese rodeo
# solo es necesario para OCP, porque ahí se necesita el VA de LA EMPRESA
# (no del sector) y OCP no publica su propio VA por separado.
#
# VERSIÓN CORREGIDA: usar B1G_total (VA real, observado) directo, y
# escalar el empleo del ECOSISTEMA por la fracción de exportaciones -
# ambos correspondientes al MISMO año (2021, el único con datos OCDE), no
# mezclando el empleo de 2023 con la fracción de otro año.

anio_check_auto <- anio_oecd_ref  # 2021: mismo año en ambos lados del cociente

fraccion_empleo_c29_c27 <- auto_exportaciones_oc |>
  filter(year == anio_check_auto) |>
  mutate(fraccion = (cableado_mmad + ensamblaje_mmad) / total_mmad) |>
  pull(fraccion)

L_ecosistema_2021 <- auto_empleo_serie$empleo[auto_empleo_serie$year == anio_check_auto]
L_auto_ajustado    <- L_ecosistema_2021 * fraccion_empleo_c29_c27

VA_c29_c27_2021 <- par_auto_v2$B1G_total  # VA real de C29+C27, dato observado de la OCDE

vpmgl_auto_check <- par_auto_v2$share_trabajo * VA_c29_c27_2021 / L_auto_ajustado
...
# Resultado esperado: ~4.704 MAD/mes vs. 5.002 de CNSS -> margen ~0,94x.
# No pasa el test en sentido estricto, pero queda dentro del margen
# razonable de error dado que se están mezclando fuentes de años distintos
# (CNSS 2020, OCDE 2021) y una fracción de empleo que es un proxy, no una
# medición directa por rama. A partir de acá NO seguir afinando el
# perímetro automotor (agregar C25/C28) solo para forzar que el margen
# cruce 1,00 - eso sería sobreajustar el resultado, no mejorarlo. Se trata
# como un empate técnico, no como un fallo del modelo.
