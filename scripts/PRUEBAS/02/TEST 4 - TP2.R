...
# pide la consigna (MFE y HO), con sub-bloques adentro de cada uno.
#
# Esta versión incorpora, además de la devolución de Gemini Spark sobre el
# borrador anterior (FPP real en el plano Q_fosfatos x Q_automotor, caja de
# asignación con VPMgL, justificación empírica de intensidades por código),
# dos cosas más:
#   (a) el chequeo de VPMgL en el punto OBSERVADO (no solo el equilibrio
#       teórico) con prueba de robustez a qué serie de empleo de OCP se usa
#       (17.000 vs. 20.000) y a usar exportación aduanera en vez de
#       ingresos totales de OCP - sub-bloque 2.4.1;
#   (b) una comparación explícita Cobb-Douglas vs. Leontief para la
#       tecnología de fosfatos (sub-bloque 2.4.2), que muestra que el
#       "equilibrio" de 75.565 trabajadores que predice el Cobb-Douglas no
#       tiene sustento técnico bajo una tecnología de coeficientes fijos
#       calibrada en el mismo punto observado - la extrapolación suave es
#       la que hay que tratar con cautela, no el hallazgo cualitativo de
#       la brecha (que sí es robusto, ver 2.4.1).
#
# Sigue sin estar corrido ni validado en R (se escribió sin acceso a R para
# probarlo) - revisar nombres de columnas de pwt10.01 al cargarlo la
# primera vez.
...
if (file.exists(objetos_tp1)) {
  ...
  # livianos (más lento: reprocesa las bases .dta de WITS y regenera los
  # gráficos del TP1).
...
# para calibrar la FPP/VPMgL empírica de los sub-bloques 2.2-2.4. Ver
...
#
# SUPUESTO EXPLÍCITO: para el sector automotor se trata producción física
# (OICA, auto_produccion) y valor exportado (Office des Changes,
# auto_exportaciones_oc) como aproximadamente equivalentes (producción ≈
# exportación). Es razonable para el modelo de ensamblaje-para-exportación
# de Marruecos: más del 85-90% de los vehículos de Renault y Stellantis en
# el país se destinan al mercado externo europeo, pero es un supuesto, no
# una identidad contable exacta - no confundir "unidades producidas" con
# "unidades exportadas" si en algún momento hace falta más precisión.
...
# Empleo directo de OCP: dos series posibles, según perímetro. Se usa
# 17.000 (directo minas/química) como principal por ser más comparable con
# el empleo automotor "directo"; 20.000 (consolidado global) queda como
# chequeo de robustez en el sub-bloque 2.4.1 - la conclusión cualitativa no
...
ocp_empleo_consolidado_alt <- 20000  # usado solo en el chequeo de robustez 2.4.1
...
# provisional) - se usa 141.763 (balanza) como principal por ser el dato
...
# Sirven acá como alpha del Cobb-Douglas (2.2/2.3/2.4) y se reutilizan en el
# sub-bloque 3.3 como aproximación de los requerimientos técnicos aL,j/aK,j
...
L_total      <- L_fosfatos_0 + L_auto_0  # 247.000 (17.000 + 230.000)
...
# Familia de curvas: una FPP recalibrada por año (no una sola curva fija),
# para mostrar el desplazamiento explícitamente - cada punto observado cae
# sobre su propia curva.
...
)  # excluye 2025 (empleo automotor proyectado) y 2020 (sin producción
# automotriz reportada por OICA)
...
# Sanity check de orden de magnitud (no reemplaza una verificación formal,
# pero descarta un error grosero de unidades): precio_fosfatos ronda los
# ~2.700-2.800 MAD/tonelada, comparable al precio internacional de la roca
# fosfórica del Pink Sheet (~320 USD/t ≈ 3.000 MAD/t, 2.3); precio_auto
# ronda los ~0,26-0,27 M MAD (~260.000 MAD, ~26.000 USD) por vehículo,
# razonable para una mezcla Dacia/Renault de gama media-baja.
...
# El "equilibrio" de 2.4 (L_fosfatos_eq) queda lejísimos del empleo real de
# OCP (L_fosfatos_0 = 17.000). Antes de usar esa cifra en la presentación,
# hay que mostrar la VPMgL en el punto REAL, y confirmar que la conclusión
# (fosfatos genera mucho más valor por trabajador adicional que automotor)
# no depende de decisiones arbitrarias de qué serie/perímetro usar.
...
# Robustez 1: usar 20.000 (consolidado) en vez de 17.000 (directo minas/química)
...
# Robustez 2: usar exportación aduanera de fosfatos (Office des Changes) en
# vez de ingresos totales de OCP (que incluyen ventas domésticas y
# downstream, no solo exportación de roca/fertilizantes)
...
# La brecha se mueve entre ~3.2x y ~3.9x según el supuesto, pero la
# conclusión cualitativa (fosfatos genera varias veces más valor por
# trabajador adicional que automotor) es robusta a las tres variantes.
...
g_caja_asignacion <- ggplot(caja_asignacion, aes(x = L_fosfatos)) +
  ...
  # cuando el trabajo de un sector -> 0); ver
  # nota de la caption sobre lo que queda afuera
  scale_color_manual(values = paleta_sectores) +
  labs(title = "Caja de asignación del trabajo — MFE",
       ...
       caption = paste0(
         ...
         round(brecha_vpmgl_obs, 1), "x). La distancia al equilibrio teórico NO es un error de unidades ",
         "(ver tabla_robustez_vpmgl) - refleja segmentación institucional del mercado de trabajo, no falta ",
         "de ajuste (ver 2.6/Bloque 4). Curvas recortadas en VPMgL=2 (colas asintóticas fuera de escala)."
       )) +
  theme_tp1()
g_caja_asignacion
...
# L_fosfatos_eq (75.565) es una extrapolación del Cobb-Douglas ~4,4 veces
# más allá de cualquier nivel de empleo jamás observado en fosfatos
# (17.000-20.000). Antes de citar ese número puntual, se lo compara contra
# lo que predeciría una tecnología de coeficientes fijos (Leontief)
# calibrada en el mismo punto observado - más plausible para una planta
# minera con infraestructura fija (tubería de mineral Khouribga-Jorf
# Lasfar, dragalinas, capacidad de procesamiento instalada): más allá de
# cierto punto, agregar trabajo no aumenta la producción (PMgL = 0), no
# decae suavemente hacia cero como en Cobb-Douglas.

aL_fosfatos_leontief <- L_fosfatos_0 / Q_fosfatos_0  # trabajadores por mil toneladas, calibrado en 2023
Q_max_fosfatos <- max(fosfatos_produccion$produccion_mt)  # techo de capacidad: pico histórico 2019-2025
...
# Q_max_fosfatos es apenas ~15% mayor al Q observado en 2023 (pico
# histórico 2019-2025 vs. producción 2023), así que L_capacidad_leontief
# queda muy cerca de L_fosfatos_observado_2023 y MUY por debajo de
# L_fosfatos_eq. Conclusión: bajo una tecnología de coeficientes fijos
# consistente con cómo opera la planta hoy, no hay margen técnico para
# emplear productivamente 75.000 trabajadores en fosfatos, ni siquiera
# operando a capacidad máxima histórica. La cifra de Cobb-Douglas no es una
# predicción operativa creíble - es un artefacto de asumir sustituibilidad
# suave entre capital y trabajo donde la tecnología real es mucho más
# rígida. Esto no invalida el hallazgo cualitativo de la brecha (2.4.1,
# robusto a como se lo mida) - lo que no hay que hacer es citar 75.565
# como si fuera una predicción operativa.
...
g_leontief_vs_cd <- ggplot(leontief_vs_cd, aes(x = L_fosfatos)) +
  ...
  labs(title = "¿Qué tecnología describe mejor a la minería de fosfatos?",
       ...
       caption = paste0(
         ...
         "). Línea rayada: equilibrio que predice Cobb-Douglas (", format(round(L_fosfatos_eq), big.mark = "."),
         "). Bajo Leontief la producción no puede superar el pico histórico (",
         format(Q_max_fosfatos, big.mark = "."), " mil t) sin importar cuánto trabajo se agregue: ese ",
         "equilibrio no tiene sustento técnico bajo coeficientes fijos calibrados en el mismo punto observado."
       )) +
  theme_tp1()
g_leontief_vs_cd
...
# MFE (inciso d) contra lo efectivamente ocurrido, y para explicar la
# brecha de 2.4.1/2.4.2: segmentación institucional, no ausencia de
...
brecha_valor_trabajador  # respalda tomar fosfatos como intensivo en el
# factor específico y automotor como el sector
# relativamente absorbente de trabajo abundante -
# ver 2.4.1 para el mismo chequeo ya llevado a
# VPMgL con el alpha de io_coeficientes.
...
# Cobb-Douglas de 2.2/2.3/2.4 son, leídos al revés, una aproximación de los
# requerimientos técnicos aL,j/aK,j que pide el estilo del Ejercicio 11 de
# la guía teórica. No reemplazan una matriz insumo-producto real por rama
...
# junto con la brecha de valor por trabajador de 3.1.
...
ied_origen_pais_2020 <- tibble(
  ...
  stock_mmad = c(194400, 132400, 53100, 27500, 22500),
...
# Candidato natural para el inciso 4b junto con la brecha de renta de 2.4.1:
...
# parte esta política de subsidio al consumo básico para los asalariados
# del resto de la economía (incluido el automotor, con salarios cercanos
# al SMIG - ver salarios_cnss_sector_2020).
...
# Industrial). Texto para la diapositiva, no serie.
...
#    agregada) - mismo motivo estructural. salarios_cnss_sector_2020 con
#    "Industrie" es el mejor proxy disponible.
# 3. Variante "Opción B" sugerida por Gemini para 3.1: calcular alpha
#    directamente desde Valor Agregado y Remuneración de Asalariados de
#    OECD TiVA/ICIO para Marruecos (sectores B y C29), en vez de vía
#    io_coeficientes del HCP. Más riguroso, pero requiere descargar esa
#    base aparte - no implementado.
# 4. aL_fosfatos_leontief (2.4.2) se calibra con un solo año (2023) y
#    Q_max_fosfatos usa el pico histórico de la serie 2019-2025 como proxy
#    de capacidad instalada - es una aproximación razonable, no un dato de
#    ingeniería de planta. Si en algún momento aparece la capacidad
#    nominal real de OCP (toneladas/año de diseño), reemplazar
#    Q_max_fosfatos por ese valor.
# 5. anio_ref_fpp está fijo en 2023 (2.3/2.4) - si conviene otro año de
#    referencia (por disponibilidad de datos o narrativa), es la única
#    variable que hay que cambiar para recalcular la FPP, la caja de
#    asignación y la comparación Leontief/Cobb-Douglas completas.